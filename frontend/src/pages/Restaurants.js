...
import defaultImage from '../assets/images/restaurants.jpg';
...
const Restaurants = () => {
    ...
    useEffect(() => {
        if (coordinates) {
            ...
            // Fetch nearby restaurants with detailed information
            axios.get(`/locationSearch/api/sparql/nearby-restaurants`, { params: { latitude: lat, longitude: long } })
                .then(async response => {
                    const restaurantsWithPhotos = await Promise.all(response.data.map(async restaurant => {
                        const photoUrl = await fetchRestaurantPhoto(restaurant.placeId);
    ...
    const fetchRestaurantPhoto = async (placeId) => {
        try {
    ...
    const renderRestaurant = (restaurant, index) => (
        <MDBCol key={index} md="3" className="mb-4">
            <MDBCard>
                <MDBCardImage src={restaurant.photoUrl || defaultImage} position="top" alt={restaurant.placeLabel} style={{ height: '145px', objectFit: 'cover' }} />
                <MDBCardBody>
                    <MDBCardTitle>{restaurant.placeLabel}</MDBCardTitle>
                    <MDBCardText>{restaurant.description}</MDBCardText>
                    <MDBCardText><strong>Distance:</strong> {restaurant.distance}m</MDBCardText>
                    <a href={`https://www.google.com/maps/search/?api=1&query=${restaurant.placeLabel}`} target="_blank" rel="noopener noreferrer" className="category-link">
                        View on Google Maps
                    </a>
                </MDBCardBody>
            </MDBCard>
    ...
    const renderList = (title, items) => (
        <MDBContainer>
            ...
            <MDBRow>
                {items.slice(0, 4).map((item, index) => renderRestaurant(item, index))}
            </MDBRow>
            <MDBRow>
                {items.slice(4, 8).map((item, index) => renderRestaurant(item, index))}
            </MDBRow>
            <MDBRow className="justify-content-center">
                {items.slice(8, 10).map((item, index) => renderRestaurant(item, index))}
            </MDBRow>
        </MDBContainer>
    );
